  ...
  return (
    <div className={`relative ${className}`} ref={ref}>
      ...
      >
        ...
        {/* Flèche SVG */}
        <svg
          className={`absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[var(--ink-subtle)] transition-transform duration-200 ${
            open ? "rotate-180" : ""
          }`}
          viewBox="0 0 24 24"
          fill="none"
      ...
      {open && (
        <>
          ...
          {/* Liste des options */}
          <ul
            className="absolute z-50 mt-1.5 max-h-56 w-full overflow-auto rounded-2xl border border-[var(--accent)]/20 bg-white dark:bg-[#1a1d1e] shadow-lg shadow-black/5 dark:shadow-black/30 py-1 text-sm"
          >
            {options.map((opt) => {
